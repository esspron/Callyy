...
// Solution page data
export const getSolutionData = (category: string, slug: string) => {
  ...
  const industryFeatures = {
    healthcare: [
      { title: 'HIPAA Compliant', description: 'All calls are encrypted and stored securely.', icon: '🔒' },
      { title: 'Appointment Scheduling', description: 'Automated booking for patients.', icon: '📅' },
      { title: 'Prescription Reminders', description: 'Proactive outreach for medication.', icon: '💊' },
    ],
    'financial-services': [
      { title: 'KYC Verification', description: 'Automated identity verification calls.', icon: '✓' },
      { title: 'Payment Reminders', description: 'Gentle collection and reminder calls.', icon: '💰' },
      { title: 'Fraud Alerts', description: 'Real-time suspicious activity notifications.', icon: '🚨' },
    ],
  }

  const useCaseFeatures = {
    'lead-qualification': [
      { title: 'Smart Scoring', description: 'AI-powered lead scoring based on conversation.', icon: '📊' },
      { title: 'CRM Integration', description: 'Auto-sync qualified leads to your CRM.', icon: '🔄' },
      { title: 'Follow-up Scheduling', description: 'Book demos automatically for hot leads.', icon: '📞' },
    ],
    'ai-customer-service': [
      { title: '24/7 Availability', description: 'Never miss a customer query.', icon: '🌙' },
      { title: 'Multi-language Support', description: 'Serve customers in their language.', icon: '🌍' },
      { title: 'Escalation Handling', description: 'Smart handoff to human agents.', icon: '👥' },
    ],
  }

  // Get features based on category and slug
  let features = [
    { title: 'Feature 1', description: `Specific capability designed for ${title}.`, icon: '✨' },
    { title: 'Feature 2', description: `Enhanced automation for ${title} workflows.`, icon: '⚡' },
    { title: 'Feature 3', description: `Analytics and insights for ${title}.`, icon: '📈' },
  ]

  if (category === 'industry' && industryFeatures[slug as keyof typeof industryFeatures]) {
    features = industryFeatures[slug as keyof typeof industryFeatures]
  } else if (category === 'use-case' && useCaseFeatures[slug as keyof typeof useCaseFeatures]) {
    features = useCaseFeatures[slug as keyof typeof useCaseFeatures]
  }
  ...
  return {
    ...
    slug,
    title,
    description: `Discover how Voicory transforms ${title} with advanced voice AI agents. Automate calls, improve customer experience, and scale your operations effortlessly.`,
    features,
  }
}
