...
class WebRTCVoiceSession {
    ...
    isMeaningfulSpeech(transcript) {
        ...
        // Common filler words and non-speech sounds (exact matches only)
        const PURE_FILLERS = new Set([
            // English fillers
            'um', 'uh', 'ah', 'eh', 'oh', 'hm', 'hmm', 'mm', 'mhm', 'uh-huh', 'nah',
            // Hesitations
            'er', 'err', 'erm',
            // Throat sounds / non-words
            'ahem', 'cough', 'sigh', 'laugh', 'haha', 'hehe',
        ]);
        
        // If it's EXACTLY a filler word, filter it
        if (PURE_FILLERS.has(cleaned)) {
            return false;
        }
        
        // Multi-word = meaningful (even "oh sorry" or "wait stop")
        const words = cleaned.split(/\s+/).filter(w => w.length > 0);
        if (words.length >= 2) {
            return true;
        }
        
        // Single word: must be at least 3 chars and look like a real word
        if (words.length === 1) {
            const word = words[0];
